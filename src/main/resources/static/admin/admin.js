  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    //show total Distributor count  --------------------------------------------------------
    document.addEventListener('DOMContentLoaded', function() {
    const userEmail = sessionStorage.getItem('userEmail');
        if (userEmail) {
               document.getElementById('userEmail').textContent = userEmail;
               fetchTotalDistributors();
               fetchTotalRetailers();
           } else {
               console.warn('No userEmail found in session storage');
           }
    });

    function fetchTotalDistributors() {
    ...
    function fetchTotalRetailers() {
        fetch('http://localhost:8080/api/admin/retailer/list')
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
