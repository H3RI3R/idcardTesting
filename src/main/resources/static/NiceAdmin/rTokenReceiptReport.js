...
document.addEventListener('DOMContentLoaded', function () {
    // Fetch the session data (userEmail)
    const userEmail = sessionStorage.getItem('userEmail');

    // Fetch data from the API
    fetch(`${API_URL}/api/admin/token/transactions?email=${userEmail}`)
        .then(response => response.json())
        .then(data => {
        // Get the table body element
        const tableBody = document.getElementById('retailerTableBody');
        ...
        data.forEach(transaction => {
            ...
            dateCell.textContent = transactionDate.toLocaleString(); // Format date and time
